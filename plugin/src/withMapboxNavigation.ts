...
let pkg: { name: string; version?: string } = {
  name: "@comediadesign/react-native-mapbox-navigation",
};
try {
  pkg = require("@comediadesign/react-native-mapbox-navigation/package.json");
} catch {
  // empty catch block
}
...
const withAndroidPropertiesDownloadToken: ConfigPlugin<
  MapboxNavigationPlugProps
> = (config, { RNMBNAVDownloadToken }) => {
...
const withAndroidPropertiesPublicToken: ConfigPlugin<
  MapboxNavigationPlugProps
> = (config, { RNMBNAVPublicToken }) => {
  const key = "MAPBOX_ACCESS_TOKEN";

  if (RNMBNAVPublicToken) {
    return withAndroidManifest(config, async (config) => {
      // Modifiers can be async, but try to keep them fast.
      config.modResults = await setMetaDataConfigAsync(
        config,
        config.modResults,
        key,
        RNMBNAVPublicToken
...
const addLibCppFilter = (appBuildGradle: string): string => {
  ...
  return mergeContents({
    tag: `@comediadesign/react-native-mapbox-navigation-libcpp`,
    src: appBuildGradle,
...
const addMapboxMavenRepo = (projectBuildGradle: string): string => {
  if (
    projectBuildGradle.includes("api.mapbox.com/downloads/v2/releases/maven")
  ) {
  ...
  return mergeContents({
    tag: `@comediadesign/react-native-mapbox-navigation-v2-maven`,
...
const withAndroidAppGradle: ConfigPlugin<MapboxNavigationPlugProps> = (
  config
) => {
...
const withAndroidProjectGradle: ConfigPlugin<MapboxNavigationPlugProps> = (
  config
) => {
